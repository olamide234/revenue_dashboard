...
export default function FilterDialog({
  ...
  const [openDropDownStatus, setOpenDropDownStatus] =
    ...
    const applyFilter= ()=> {
      const allFilters = {...dateFilters, transaction_type: openDropDownType.value, transaction_status: openDropDownStatus.value,}
      handleFilter(allFilters)
    }
    const clearFilter = ()=> {
      const allFilters = {start_date: undefined, end_date: undefined, transaction_type: [], transaction_status: [],}
      setDateFilters({});
      setOpenDropDownType({
        state: false,
        value: [],
  ...
  const activeButtons = dateFilters?.end_date || openDropDownType.value.length > 0 || openDropDownStatus.value.length > 0;
  const inactiveApplyButton = 'bg-[#DBDEE5] text-white';
  const activeApplyButton = 'bg-[#131316] text-white';
  return (
    <div className="shadow-dialogShadow absolute bottom-2 right-2 top-2 w-1/3 min-w-[28.5rem] rounded-[20px] bg-white">
      <div
        onClick={() => {
          openDropDownType &&
      ...
      <div className="absolute bottom-0 flex w-full gap-3 rounded-b-[20px] border-2 border-white px-6 py-5">
        <button disabled={!activeButtons} onClick={clearFilter} className="w-1/2 rounded-[100px] border border-[#EFF1F6] px-6 py-3 font-semibold text-[#131316] hover:bg-[#EFF1F6]">
          Clear
        </button>
        <button
